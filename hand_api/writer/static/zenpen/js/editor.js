var editor = (function() {
	...
	// localStorage Keys (path-specific)
	...
	function init() {
		...
		if ( supportsHtmlStorage() ) {
			localStorageKeys = getStorageKeys();
			loadState();
		}
	}
	...
	function getStorageKeys () {
		...
		return { 
				'header': path + '_header', 
				'content': path + '_content',
				'last_update': path + '_last_update'
				};
	}

	function saveState( event ) {
		localStorage[ localStorageKeys['header'] ] = headerField.innerHTML;
		localStorage[ localStorageKeys['content'] ] = contentField.innerHTML;
		localStorage[ localStorageKeys['last_update'] ] = new Date().getTime();
	}

	function loadState() {

		if ( localStorage[ localStorageKeys['header'] ] ) {
			headerField.innerHTML = localStorage[ localStorageKeys['header'] ];
		}

		if ( localStorage[ localStorageKeys['content'] ] ) {
			contentField.innerHTML = localStorage[ localStorageKeys['content'] ];
		}
	}
